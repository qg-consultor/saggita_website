/* =========================================
   Carrusel de la Galería "Proyecto"
   ========================================= */
(function () {
...
(function () {
    ...
    if (protoNextBtn && protoPrevBtn && protoGallery) {
        protoNextBtn.addEventListener('click', () => {
            ...
            protoGallery.scrollBy({ left: cardWidth + 20, behavior: 'smooth' });
        });
...
(function () {
    ...
    if (fpNextBtn && fpPrevBtn && fpGallery) {
        fpNextBtn.addEventListener('click', () => {
            ...
            fpGallery.scrollBy({ left: cardWidth + 30, behavior: 'smooth' });
        });
...
/* =========================================
   LAZY LOADING DE IFRAMES (IntersectionObserver)
   Carga el iframe de video "Proyecto" y el
   de Google Maps solo cuando entran al viewport.
   ========================================= */
(function () {
...
/* =========================================
   ...
   Inicialización via IntersectionObserver
   (reemplaza setTimeout 2000ms)
   ========================================= */
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Inicializar lotes SVG via IntersectionObserver ---
    // Se activa cuando el mapa entra al viewport, no con setTimeout fijo
    let mapInitialized = false;

    function initMapShapes() {
    ...
    if (masterPlanSection) {
        ...
        }, { rootMargin: '300px 0px' }); // Pre-init antes de llegar

        mapObserver.observe(masterPlanSection);
    }

    // --- Tooltip de lotes (click) ---
    document.addEventListener('click', (e) => {
        ...
        if (lote) {
            ...
            ttTitle.textContent = lote.getAttribute('data-title');
            ttDesc.innerHTML = `Construcción: ${lote.getAttribute('data-const')} m²<br>Terreno: ${lote.getAttribute('data-terr')} m²<br>Status: ${lote.getAttribute('data-status')}`;

            const status = lote.getAttribute('data-status');
            ttBtn.style.display = (status && status.toLowerCase() !== 'disponible') ? 'none' : 'inline-block';
    ...
    // --- Botón del tooltip: scroll suave a #contacto ---
    if (ttBtn) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Zoom Explorer para imágenes de planos ---
...
/* =========================================
   PAGE LOADER — Se oculta tras 2.5 segundos
   (tiempo mínimo garantizado para la animación)
   ========================================= */
(function () {
    window.addEventListener('load', function () {
        ...
        // 2.5s fijos — garantiza que se vea la animación del loader
